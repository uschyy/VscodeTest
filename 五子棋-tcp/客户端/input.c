...
// extern int location_i[255];
// extern int location_j[255];
// extern int take_back_step_server;
// extern int take_back_step_client;

// take_back_step_server=1;
//是否胜利
	...
	while(1)
	{
		...
		if(abs(x1-x0)<=50 && abs(y1-y0)<=50)//第一次点击的坐标和抬起时的坐标相差不大，视为点击
		{
			...
			{	
				new_x=x0;
				new_y=y0;
				// location_i[take_back_step_client]= new_x;
				// location_j[take_back_step_client] = new_y;
				// take_back_step_client+=2;
				printf("input---%d %d",new_x,new_y);
				close(fd);
				return click_down;
			}	
		}
...
int process_input_yse_or_no(void)//确认与否
{
	int x0,y0,x1,y1,start,end;
	int fd = open("/dev/input/event0", O_RDONLY);
	if(fd == -1)
...
// extern int take_back_array[15][15][15];
//表示下棋了
extern int reveive_server_move_flag;
